...
setwd("~/Documents/Princeton/tiago_rotation/clock_models/RevBayes/RB_simulations_sampled25_output")
library(RevGadgets)
library(ggplot2)
library(ggtree)
...
## Plot MAP tree
tree <- readTrees("rel_mvBM_sampled25_MAP_job1.tre")
plotTree(tree, color_branch_by="branch_rates", tip_labels_size=2)

## Plot mean rates from the .trees file
### Read in the trace
trace <- readTrace("rel_mvBM_sampled25_job1.log", burnin=0.1) 

### Read in the trees
trees <- readTrees("rel_mvBM_sampled25_job1.trees", tree_name="Tree")

### use processBranchData to combine them
data_tree <- processBranchData(trees, trace, burnin=0.1, 
                               parnames=paste0('branch_rates[', 1:340,']'),
                               summary="mean")
...
saveRDS(data_tree, "results/data_tree_sampled25.rds")
...
sim.data.path <- "../../../simulating/simulated_data_v2/BM10_conditional_constPshift_sampled25perc.rds"

## Add node labels to the data_tree using the node_name_map file
### Read in the name map file
output.node.label.df <- read.table("node_name_map_job1.txt",
                                   sep="\t")
colnames(output.node.label.df) <- c("index", "label")

### Add labels to the data_tree object
data_tree <- readRDS("results/data_tree_sampled25.rds")
...
trace <- readTrace("rel_mvBM_sampled25_job1.log", burnin=0.1) 
data_tree@data$mean_branch_rates <- apply(trace[[1]], 2, mean)[paste0('branch_rates[', 1:340,']')][data_tree@data$index]
# Save the @data (input to get_combined_tree)
write.csv(data_tree@data, "results/data_tree_sampled25_processed.csv", row.names=FALSE, quote=FALSE)
