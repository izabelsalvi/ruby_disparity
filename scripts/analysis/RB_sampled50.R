...
setwd("~/Documents/Princeton/tiago_rotation/clock_models/RevBayes/RB_simulations_sampled50_output")
library(RevGadgets)
library(ggplot2)
library(ggtree)
...
## Plot MAP tree
tree <- readTrees("rel_mvBM_sampled50_MAP_job2.tre")
plotTree(tree, color_branch_by="branch_rates", tip_labels_size=2)

## Plot mean rates from the .trees file
### Read in the trace
trace <- readTrace("rel_mvBM_sampled50_job2.log", burnin=0.1) 

### Read in the trees
trees <- readTrees("rel_mvBM_sampled50_job2.trees", tree_name="Tree")
...
saveRDS(data_tree, "results/data_tree_sampled50.rds")
...
sim.data.path <- "../../../simulating/simulated_data_v2/BM10_conditional_constPshift_sampled50perc.rds"
...
data_tree <- readRDS("results/data_tree_sampled50.rds")
...
trace <- readTrace("rel_mvBM_sampled50_job2.log", burnin=0.1) 
data_tree@data$mean_branch_rates <- apply(trace[[1]], 2, mean)[paste0('branch_rates[', 1:653,']')][data_tree@data$index]
write.csv(data_tree@data, "results/data_tree_sampled50_processed.csv", row.names=FALSE, quote=FALSE)
...
ggtree(combined_tree, aes(color = rates.RBrelNorm), layout="circular", size=1) +
  ...
  ggtitle("RevBayes 50% Sampled Tree")
